...
class KittiOdomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(
            self.root_dir, "sequences", seq, "image_2", rgb_name + self.suf
        )
...
class DatasetLidarCameraKittiRaw(Dataset):
    def __init__(
        ...
        split="val",
        device="cpu",
        val_sequence="2011_09_26_drive_0117_sync",
    ):
        ...
        for i in range(len(date_list)):
            ...
            calib = {
                "K2": data.calib.K_cam2,
                "K3": data.calib.K_cam3,
                "RT2": data.calib.T_cam2_velo,
                "RT3": data.calib.T_cam3_velo,
            }
        ...
        for seq in seq_list:
            if not os.path.isdir(os.path.join(dataset_dir, date, seq)):
                continue
            image_list = os.listdir(
                os.path.join(dataset_dir, date, seq, "image_02/data")
            )
            image_list.sort()
            ...
            for image_name in image_list:
                ...
                if seq == val_sequence and (not split == "train"):
                    self.all_files.append(
                        os.path.join(
                            date, seq, "image_02/data", image_name.split(".")[0]
                        )
                    )
                elif (
                    (not seq == val_sequence)
                    and split == "train"
                    and seq not in test_list
                ):
                    self.all_files.append(
                        os.path.join(
                            date, seq, "image_02/data", image_name.split(".")[0]
        ...
        if split == "val" or split == "test":
            val_RT_file = os.path.join(
                dataset_dir, f"val_RT_seq{val_sequence}_{max_r:.2f}_{max_t:.2f}.csv"
            )
            if os.path.exists(val_RT_file):
                print(f"VAL SET: Using this file: {val_RT_file}")
                df_test_RT = pd.read_csv(val_RT_file, sep=",")
                ...
                for i in range(len(self.all_files)):
                    ...
                    self.val_RT.append(
                        [float(i), transl_x, transl_y, transl_z, rotx, roty, rotz]
                    )
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(
            self.root_dir, date, seq, "image_02/data", rgb_name + ".jpg"
        )  # png
        lidar_path = os.path.join(
            self.root_dir, date, seq, "velodyne_points/data", rgb_name + ".bin"
        )
        lidar_scan = np.fromfile(lidar_path, dtype=np.float32)
        pc = lidar_scan.reshape((-1, 4))
        valid_indices = pc[:, 0] < -3.0
