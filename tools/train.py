...
def main(_config, _run, seed):
    ...
    model_savepath = os.path.join(
        _config["checkpoints"], "val_seq_" + _config["val_sequence"], "models"
    )
    ...
    if _config["loss"] == "simple":
        ...
        loss_fn = CombinedLoss(
            _config["rescale_transl"],
            _config["rescale_rot"],
            _config["weight_point_cloud"],
        )
    else:
    ...
    if _config["optimizer"] == "adam":
        optimizer = optim.Adam(
            parameters, lr=_config["BASE_LEARNING_RATE"], weight_decay=5e-6
        )
    ...
    for epoch in range(starting_epoch, _config["epochs"] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if batch_idx % _config["log_frequency"] == 0:
                ...
                train_writer.add_scalar(
                    "Loss_Total", loss["total_loss"].item(), train_iter
                )
                train_writer.add_scalar(
                    "Loss_Translation", loss["transl_loss"].item(), train_iter
                )
                train_writer.add_scalar(
                    "Loss_Rotation", loss["rot_loss"].item(), train_iter
                )
                if _config["loss"] == "combined":
                    train_writer.add_scalar(
                        "Loss_Point_clouds",
                        loss["point_clouds_loss"].item(),
                        train_iter,
                    )
        ...
        print("------------------------------------")
        _run.log_scalar(
            "Total training loss", total_train_loss / len(dataset_train), epoch
        )
